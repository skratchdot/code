...
const solve2 = (lines) => {
  const cards = lines.map((line) => {
    ...
    debug({ name, cards, line });
    return cards.map(parseFloat);
  });
  const [player1, player2] = cards;
  debug({ cards });

  // returns true if player1 wins
  const playGame = (player1, player2, isFirst) => {
    let round = 0;
    let playingGame = player1.length > 0 && player2.length > 0;
    ...
    // store previous rounds (as a hash for performance)
    while (playingGame) {
      round++;
      hand = player1.join(',') + '|' + player2.join(',');
    ...
    return winner(player1.length > player2.length);
  };

  const result = playGame(player1, player2, true);

  const getScore = (arr) =>
    arr.reduce(
      (prev, curr, index, arr) => prev + curr * (arr.length - index),
      0
    );

  return result.player1Wins
    ? getScore(result.player1)
    : getScore(result.player2);
};

run(__dirname, 'inputtest', solve2, 291, { split: '\n\n' });
run(__dirname, 'inputtest2', solve2, 105, { split: '\n\n' });
run(__dirname, 'input', solve2, 29177, { split: '\n\n' });
