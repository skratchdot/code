...
const getLoopSize = (subjectNumber, publicKey) => {
  let value = 1;
  let loopSize = 0;
...
const solve1 = (lines) => {
  const [cardPublicKey, doorPublicKey] = lines.map(parseFloat);
  const cardLoopSize = getLoopSize(7, cardPublicKey);
  const doorLoopSize = getLoopSize(7, doorPublicKey);
  debug({ cardLoopSize, doorLoopSize });
  const encryptionKey = getEncryptionKey(doorPublicKey, cardLoopSize);
  debug({ encryptionKey });
  return encryptionKey;
};

run(__dirname, 'inputtest', solve1, 14897079);
run(__dirname, 'input', solve1, 11707042);
