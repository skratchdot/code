...
const solve1 = (lines) => {
  ...
  const errors = nearbyTickets.reduce((errors, nums) => {
    nums.forEach((num) => {
      let valid = false;
      rules.forEach((rule) => {
  ...
  const ticketScanningErrorRate = errors.reduce((a, b) => a + b, 0);

  debug({ rules, yourTicket, nearbyTickets, errors, ticketScanningErrorRate });
  return ticketScanningErrorRate;
};

run(__dirname, 'inputtest', solve1, 71, { split: '\n\n' });
run(__dirname, 'input', solve1, 27898, { split: '\n\n' });
